...
def transform_media(match):
    ...
    media_slug = (
        re.search(r"\[MEDIA.*?\](.*?)\[\/MEDIA\]", substring, re.IGNORECASE | re.DOTALL)
        .group(1)
        .strip()
    )
...
def transform_image(match):
    ...
    image_url = (
        re.search(r"\[IMG.*?\](.*?)\[\/IMG\]", substring, re.IGNORECASE | re.DOTALL)
        .group(1)
        .strip()
    )

    return (
        f'<IMG alt="image" src="{image_url}"><s>![</s>image<e>]({image_url})</e></IMG>'
    )
...
def transform_url(match):
    ...
    link_display = re.search(
        r"\[URL.*?\](.*?)\[\/URL\]", substring, re.IGNORECASE | re.DOTALL
    ).group(1)
    ...
    old_forum_search = re.search(
        r"forum[s]?\.meepcraft\.com\/threads\/[^\.]*\.([^\/]*)\/?", link_url
    )
    if old_forum_search:
...
def wrap_code(match):
    ...
    front_tag_search = re.search(r"\[({}=.*?)\]".format(code), substring, re.IGNORECASE)
